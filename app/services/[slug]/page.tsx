...
export default async function ServiceDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section"><div className="container detail-hero"><article>{service.image?.asset?.url && <div className="detail-media"><Image src={urlFor(service.image).width(760).height(1013).fit('crop').url()} alt={service.image.alt || service.title} width={760} height={1013} priority /></div>}<div className="kicker">About this treatment</div><h2>A considered pathway, not a one-size-fits-all procedure.</h2><p style={{fontSize:17}}>{service.body}</p><div className="feature-list"><div><span className="check">1</span><span><strong>Consultation</strong><br/><span className="muted">Assess goals, contraindications and suitability.</span></span></div><div><span className="check">2</span><span><strong>Treatment plan</strong><br/><span className="muted">Build a phased plan around skin, anatomy and priorities.</span></span></div><div><span className="check">3</span><span><strong>Review</strong><br/><span className="muted">Review progress and refine the plan over time.</span></span></div></div></article><aside className="detail-aside"><div className="kicker">Ready to start?</div><h3>Discuss your goals with a clinician.</h3><p>Use the enquiry form and the team can guide the next step.</p><Link href="/contact" className="btn btn-primary" style={{width:'100%'}}>Book consultation</Link><hr/><small className="muted">This page does not constitute medical advice.</small></aside></div></section>
  </>
}
