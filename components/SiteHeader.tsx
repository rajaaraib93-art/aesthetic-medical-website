...
export function SiteHeader({ siteName = 'IARM' }: { siteName?: string }) {
  const [open, setOpen] = useState(false)
  const [first, ...rest] = siteName.split(' ')
  return (
    <>
      <div className="announcement">Medical aesthetics & regenerative care · Book a consultation</div>
      <header className="header">
        <div className="container navbar">
          <Link href="/" className="brand" onClick={() => setOpen(false)}>
            <span className="brand-mark">{first.charAt(0)}</span>
            <span>{first} <span style={{fontWeight:500}}>{rest.join(' ')}</span></span>
          </Link>
